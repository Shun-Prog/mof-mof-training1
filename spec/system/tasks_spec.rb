...
RSpec.describe 'Tasks', type: :system do
  ...
  describe '一覧' do
    ...
    context '初期表示時' do
      ...
      let!(:task_add_1day) do
        create(:task, created_at: Time.current + 1.days, user: user)
      end
      ...
      it '作成日の降順で並ぶ' do
        expect(rows[0].find('.task_created_at').text).to eq I18n.l(
             task_add_1day.created_at
           ).to_s
        expect(rows[1].find('.task_created_at').text).to eq I18n.l(
             task.created_at
           ).to_s
      end
    end
    ...
    context '終了期限でソートした時' do
      let!(:task) { create(:task, user: user) }
      let!(:task_add_1day) do
        create(:task, expired_at: Time.current + 1.days, user: user)
      end
      ...
      it '昇順で並ぶ' do
        ...
        sleep 0.5
        expect(rows[0].find('.task_expired_at').text).to eq I18n.l(
             task.expired_at
           ).to_s
        expect(rows[1].find('.task_expired_at').text).to eq I18n.l(
             task_add_1day.expired_at
           ).to_s # DOMを待つ
      end
    ...
    context '優先順位でソートした時' do
      ...
      it '昇順で並ぶ' do
        ...
        sleep 0.5
        expect(rows[0].find('.task_priority').text).to eq task_priority_low
             .priority_i18n # DOMを待つ
      end
      ...
      it '降順で並ぶ' do
        ...
        expect(rows[0].find('.task_priority').text).to eq task_priority_high
             .priority_i18n
      end
    end
    ...
    context 'ラベルで検索した時' do
      ...
      it '検索した値で表示される' do
        ...
        click_button '検索'
        expect(rows[0].find('.task_labels').text).to have_content task_labels[0]
                       .label.name
      end
    end
  end
    ...
    context 'タスク名を入力した時' do
      ...
      it '作成できる' do
        ...
        expect(page).to have_content I18n.t(
                       :ready,
                       scope: %i[enums task status]
                     )
        expect(page).to have_content I18n.t(
                       :low,
                       scope: %i[enums task priority]
                     )
        expect(page).to have_selector '.task_expired_at',
                      text: I18n.l(expired_at).to_s
        expect(page).to have_selector '.task_created_at',
